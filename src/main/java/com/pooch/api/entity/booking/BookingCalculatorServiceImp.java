...
public class BookingCalculatorServiceImp implements BookingCalculatorService {
    ...
    @Override
    public BookingCostDetails calculateBookingDetailCosts(BookingCalculatorSender sender, Groomer groomer, Parent parent, Double pickUpCost, Double dropOffCost, LocalDateTime startDateTime,
            LocalDateTime endDateTime, List<PoochBookingCreateDTO> pooches) {
        ...
        Double careServicesCost = calculateCareServicesCost(sender, groomer, parent, numberOfDays, pooches);
     ...
     * careServicesCost
     */
    private Double calculateCareServicesCost(BookingCalculatorSender sender, Groomer groomer, Parent parent, int numberOfDays, List<PoochBookingCreateDTO> petCreateDTOs) {
        ...
        if (petCreateDTOs != null && petCreateDTOs.size() >= 0) {
            ...
            for (PoochBookingCreateDTO petCreateDTO : petCreateDTOs) {
                ...
                if (poochUuid != null && !poochUuid.trim().isEmpty()) {
                    ...
                    Pooch pooch = poochDAO.getByUuid(poochUuid).orElseThrow(() -> new ApiException(ApiError.DEFAULT_MSG, "Pooch not found for uuid=" + poochUuid));
                ...
                for (BookingCareServiceCreateDTO service : services) {
                    ...
                    CareService careService = careServiceDAO.getByUuidAndGroomer(careServiceUuid, groomer.getId())
                            .orElseThrow(() -> new ApiException(ApiError.DEFAULT_MSG, "service.uuid belongs to a different groomer"));

                    if (!careService.isSizeServiced(size)) {
                        throw new ApiException(ApiError.DEFAULT_MSG, "Groomer does not off service for this size", "size: " + size, "serviceName: " + careService.getName());
                    }
                    ...
                    log.info("careService.name={}, numberOfDays={}, careServicePricePerDay={}, careServicePrice={}", careService.getName(), numberOfDays, careServicePricePerDay, careServicePrice);
    ...
    private int calculateNumberOfDays(Groomer groomer, LocalDateTime startDateTime, LocalDateTime endDateTime) {
        ...
        if (!((startDateTime.toLocalTime().equals(openTime) || startDateTime.toLocalTime().isAfter(openTime))
            ...
            throw new ApiException(ApiError.DEFAULT_MSG, "startDateTime must be within open and close time", "openTime: " + openTime, "closeTime: " + closeTime,
                    "starTime: " + startDateTime.toLocalTime());
        }
        ...
        if (startDateTime.isAfter(endDateTime)) {
            throw new ApiException(ApiError.DEFAULT_MSG, "endDateTime must be greater than startDateTime");
        }
        ...
        log.info("numberOfDays={}, startDateTime={}, endDateTime={}", numberOfDays, startDateTime, endDateTime);
